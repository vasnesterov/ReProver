...
class Seq2VecEncoder:
    """Abstract class for seq2vec encoder: either premise encoder or context encoder"""
    def _encode(
...
class T5ContextEncoder(pl.LightningModule, Seq2VecEncoder):
    def __init__(
        self,
        model_name: str,
        lr: float,
...
class LlemmaPremiseEncoder(pl.LightningModule, Seq2VecEncoder):
    #### i'm here
...
class TwoEncoderPremiseRetriever(pl.LightningModule):
    def __init__(
        ...
        self.context_encoder = AutoModel.from_pretrained(context_model_name)
        self.premise_tokenizer = AutoTokenizer.from_pretrained(premise_model_name)
        self.premise_encoder = AutoModel.from_pretrained(premise_model_name)
        self.embeddings_staled = True

    @classmethod
    def load(cls, ckpt_path: str, device, freeze: bool) -> "TwoEncoderPremiseRetriever":
    ...
    def reindex_corpus(self, batch_size: int) -> None:
        ...
        for i in tqdm(range(0, len(self.corpus), batch_size)):
            ...
            tokenized_premises = self.premise_tokenizer(
    ...
    def retrieve(
        ...
        ctx_tokens = self.context_tokenizer(
