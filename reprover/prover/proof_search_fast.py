...
from reprover.generator.model import (
...
class BestFirstSearchProver:
    ...
    def search(self, repo: LeanGitRepo, thm: Theorem, pos: Pos) -> Optional[SearchResult]:
    ...
    def _step(self):
        ...
        if self.debug:
            assert all(search_node.priority >= node.priority for node in self.priority_queue)
        ...
        results = [self._run_tactic(search_node, tactic, logprob) for tactic, logprob in suggestions]
        ...
        if self.debug:
            assert self.num_expansions == sum(
                node.is_explored for node in self.nodes.values() if isinstance(node, InternalNode)
            )
...
class CpuProver(BestFirstSearchProver):
    ...
    def __init__(
        ...
        if ckpt_path is None:
            ...
            tac_gen = tacgen_class.load(ckpt_path, device=torch.device("cpu"), freeze=True)
...
class GpuProver(BestFirstSearchProver):
    ...
    def __init__(
        ...
        if ckpt_path is None:
            ...
            tacgen_class = RMTRetrievalAugmentedGenerator if use_RMT else RetrievalAugmentedGenerator
            tac_gen = tacgen_class.load(ckpt_path, device=torch.device("cuda"), freeze=True)
            if tac_gen.retriever is not None:
                if indexed_corpus_path is not None:
                    tac_gen.retriever.load_corpus(indexed_corpus_path)
                tac_gen.retriever.reindex_corpus(batch_size=32)
...
class AsyncGpuProxy:
    ...
    def __init__(
        ...
        self.model = model_class.load(ckpt_path, device=torch.device("cuda"), freeze=True)
...
class DistributedProver:
    ...
    def __init__(
        ...
        if not self.distributed:
            if ckpt_path is None:
                ...
                tac_gen = tacgen_class.load(ckpt_path, device=device, freeze=True)
                if tac_gen.retriever is not None:
                    assert indexed_corpus_path is not None
                    tac_gen.retriever.load_corpus(indexed_corpus_path)
            self.prover = BestFirstSearchProver(tac_gen, timeout, num_sampled_tactics, debug)
            return
    ...
    def search_unordered(self, repo: LeanGitRepo, theorems: List[Theorem], positions: List[Pos]) -> List[SearchResult]:
        """Parallel proof search for `theorems`. The order of the results is not guaranteed to match the order of the input."""
        if not self.distributed:
            return [self.prover.search(repo, thm, pos) for thm, pos in zip_strict(theorems, positions)]
