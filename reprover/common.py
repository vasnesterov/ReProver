...
class Premise:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.start, Pos) and isinstance(self.end, Pos) and self.start <= self.end
...
class PremiseSet:
    ...
    def __contains__(self, p: Premise) -> bool:
        return p.path in self.path2premises and p.full_name in self.path2premises[p.path]
    ...
    def from_data(cls, file_data: Dict[str, Any]) -> "File":
        ...
        for p in file_data["premises"]:
            ...
            if full_name.startswith("[") and full_name.endswith("]"):
                ...
                continue
            premises.append(Premise(path, p["full_name"], Pos(*p["start"]), Pos(*p["end"]), p["code"]))
...
class Corpus:
    ...
    def get_accessible_premise_indexes(self, path: str, pos: Pos) -> List[int]:
        return [
            ...
            if (p.path == path and p.end <= pos) or self.transitive_dep_graph.has_edge(path, p.path)
        ]
...
def _format_augmented_state(s: str, premises: List[Premise], max_len: int, p_drop: float) -> Tuple[str, int]:
    """
...
def format_augmented_state(s: str, premises: List[Premise], max_len: int, p_drop: float) -> str:
    """Format a state with retrieved premises and drop some of them with probability ``p_drop``."""
