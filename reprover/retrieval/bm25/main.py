...
from reprover.common import Context, Corpus, format_state, get_all_pos_premises
...
def _process_theorem(
    ...
    if use_all_premises:
        ...
        accessible_premise_idxs = corpus.get_accessible_premise_indexes(file_path, Pos(*thm["start"]))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Script for training the BM25 premise retriever.")
    ...
    theorems = list(
        itertools.chain.from_iterable(
            json.load(open(os.path.join(args.data_path, f"{split}.json"))) for split in ("train", "val", "test")
        )
    )
    ...
    if args.num_cpus > 1:
        ...
        preds = list(
            itertools.chain.from_iterable(
                tqdm(
                    pool.map_unordered(lambda a, thm: a.process_theorem.remote(thm), theorems),
                    total=len(theorems),
                )
            )
        )
