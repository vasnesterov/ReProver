...
from reprover.common import IndexedCorpus, format_augmented_state, remove_marks, zip_strict
...
class RMTRetrievalAugmentedGenerator(TacticGenerator, pl.LightningModule):
    def __init__(
        ...
        self.backbone = RetrievalAugmentedGenerator(
            backbone_model_name,
            lr,
            warmup_steps,
            num_beams,
    ...
    def _encode_with_memory(
        ...
        for segment in range(num_segments):
            memory_state_shape = (
                state_ids[segment].shape[0],
                self.num_memory_tokens,
                self.backbone.generator.model_dim,
            )
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if not self.skip_test_proving:
            ...
            if self.backbone.retriever is None:
                ...
                corpus_path = f"{self.trainer.log_dir}/checkpoints/indexed_corpus.pickle"
                pickle.dump(
                    IndexedCorpus(self.backbone.retriever.corpus, self.backbone.retriever.corpus_embeddings.cpu()),
                    open(corpus_path, "wb"),
                )
                cmd = f"python prover/evaluate.py --data-path {data_path} --num-cpus {self.eval_num_cpus} --num-theorems {self.eval_num_theorems} --ckpt_path {ckpt_path} --indexed-corpus-path {corpus_path}"
